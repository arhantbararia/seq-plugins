...
import (
	...
	"openwa_action/services"
	worker "openwa_action/workers"
	"time"
)
...
var (
	...
	// openwaSvc is the service for interacting with the OpenWA REST API.
	// This is initialized in main().
	openwaSvc *services.OpenWAService
...
func buildActionConfig(req SetupPayload) models.ActionConfig {
	...
	// Extract OpenWA string fields from config map
	cfg.ChatID = services.FormatChatIDList(stringField(req.Config, "chat_id"))
	cfg.Text = stringField(req.Config, "text")
	cfg.URL = stringField(req.Config, "url")
	cfg.Caption = stringField(req.Config, "caption")
	cfg.Filename = stringField(req.Config, "filename")
	cfg.Mimetype = stringField(req.Config, "mimetype")
	cfg.Latitude = stringField(req.Config, "latitude")
	cfg.Longitude = stringField(req.Config, "longitude")
	cfg.Description = stringField(req.Config, "description")
	cfg.ContactName = stringField(req.Config, "contact_name")
	cfg.ContactNumber = stringField(req.Config, "contact_number")
	cfg.TemplateName = stringField(req.Config, "template_name")
	cfg.TemplateVars = stringField(req.Config, "template_vars")
	cfg.State = stringField(req.Config, "state")
	if v, ok := req.Config["ptt"].(bool); ok {
		cfg.PTT = v
	} else if vStr, ok := req.Config["ptt"].(string); ok {
		cfg.PTT = vStr == "true"
	}
...
func handleSetup(w http.ResponseWriter, r *http.Request) {
	...
	if openwaSvc == nil {
		openwaSvc = services.NewOpenWAService()
	}
	if resultPublisher == nil {
		resultPublisher = worker.NewPublisher()
	}

	taskHandler := openwaSvc.HandleTaskRouter(provider, resultPublisher, seq, payload.ID, config)

	consumerTag := fmt.Sprintf("openwa-action-%s", payload.WorkflowID)
...
func main() {
	log.Println("[Main] Starting OpenWA Action plugin")

	// Initialise OpenWA service.
	openwaSvc = services.NewOpenWAService()

	// Register with workflow_executor in the background with retry.
	go func() {
		for i := 0; i < 10; i++ {
			if err := services.RegisterPlugin(); err == nil {
				return
			} else {
