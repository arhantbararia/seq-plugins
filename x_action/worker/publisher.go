...
// Publisher manages the RabbitMQ connection and publishes ActionResults.
...
// Publish sends an ActionResult to the ACTION_RESULT exchange
// with the workflow_id as the routing key.
func (p *Publisher) Publish(workflowID string, result models.ActionResult) error {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	log.Printf("[Publisher] Published action result task=%s workflow=%s success=%v", result.TaskID, workflowID, result.Success)
	return nil
}
