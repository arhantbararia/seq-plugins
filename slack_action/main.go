...
import (
	...
	"reflect"
	"slack_action/models"
	"slack_action/services"
	worker "slack_action/workers"
	"sync"
	"sync/atomic"
	"time"
)
...
var (
	...
	// slackSvc is the service for interacting with the Slack API.
...
func buildActionConfig(req SetupPayload) models.ActionConfig {
	...
	cfg.Channel = stringField(req.Config, "channel")
	cfg.Message = stringField(req.Config, "message")
	cfg.Attachments = stringField(req.Config, "attachments")
	cfg.Link = stringField(req.Config, "link")
...
func handleSetup(w http.ResponseWriter, r *http.Request) {
	...
	if slackSvc == nil {
		slackSvc = services.NewSlackService()
	}
	if resultPublisher == nil {
		resultPublisher = worker.NewPublisher()
	}

	taskHandler := slackSvc.HandleTaskRouter(provider, resultPublisher, seq, payload.ID, config)

	consumerTag := fmt.Sprintf("slack-action-%s", payload.WorkflowID)
...
func main() {
	log.Println("[Main] Starting Slack Action plugin")

	// Initialise Slack service.
	slackSvc = services.NewSlackService()
